...
inline HANDLE
getConsoleHandle(const std::streambuf* osbuf) noexcept
{
...
inline bool
setWinTermAnsiColors(const std::streambuf* osbuf) noexcept
{
...
inline bool
supportsAnsi(const std::streambuf* osbuf) noexcept
{
    using std::cerr;
    using std::clog;
    using std::cout;
...
template <typename T>
inline void
setWinColorNative(std::ostream& os, T const value)
{
...
inline void
setWinTermMode(const rang::winTerm value) noexcept
{
    rang_implementation::winTermMode() = value;
}

inline void
setControlMode(const control value) noexcept
{
